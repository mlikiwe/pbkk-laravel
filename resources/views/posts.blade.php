<x-layout>
    ...
    {{-- <div class="mt-6 max-w-6xl max-lg:max-w-3xl mx-auto bg-gray-800 rounded-lg">
        <article class="py-8 pl-8 max-w-screen-md border-b border-gray-300 ">
            ...
            <p class="my-4 font-light text-white">{{Str::limit($post['body'], 150)}}</p>
            <a href="/posts/{{$post['slug']}}" class="font-medium text-white hover:underline">Read More &raquo;</a>

        </article>
    </div> --}}
                <article class="p-6 bg-gray-800 rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <h2 class="mb-2 text-2xl font-bold tracking-tight text-white hover:underline"><a href="/posts/{{$post->slug}}">{{ $post->title }}</a></h2>
                    <p class="mb-5 font-light text-gray-300">{{Str::limit($post->body, 150)}}</p>
